...
derep_lengths <- read_delim(file = "https://github.com/devonorourke/tidybug/raw/master/data/databases/my.derep.lengths.txt.gz", delim = "\t")
derep_lengths$Dataset <- "derep"
palmer_lengths <- palmer_lengths[order(palmer_lengths$length),]

raw_lengths <- read_delim(file = "https://github.com/devonorourke/tidybug/raw/master/data/databases/my.raw.lengths.gz", delim = "\t", col_names = FALSE)
...
## order plot levels:
all_sumry$Dataset <- factor(all_sumry$Dataset, levels=c("derep", "raw", "palmer", "porter"))
