...
all.df.hill$Hill_value <- round(all.df.hill$Hill_value,0)
...
plot.q0 <- ggplot(q0.hill.df, aes(x=Filt, y=Hill_value, color=Filt, group=Filt, shape=Filt)) +
  ...
  geom_jitter(alpha=0.8, position=position_jitterdodge(jitter.width = 0.3)) +
  #scale_y_continuous(limits = c(0,400)) +
  facet_grid( ~ Labeler, scales = "free") +
...
plot.q1 <- ggplot(q1.hill.df, aes(x=Filt, y=Hill_value, color=Filt, group=Filt, shape=Filt)) +
  ...
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha=0.8, position=position_jitterdodge(jitter.width = 0.3)) +
  scale_y_continuous(limits = c(20,25), breaks = c(20, 22, 24)) +
  facet_grid( ~ Labeler) +
...
## plot; save as 10_mockAlpha_HillNumbers; export at 900x900
plot_grid(plot.q0, plot.q1, plot.q2, ncol=1, rel_heights = c(1.3, 1, 1))
  ## alternatively, we could try to scale the plot a bit differently so that the vsearch q=0 outliers don't squash the dada2/deblur q=0 vals:

q0.notV <- ggplot(q0.hill.df %>% filter(Method != "vsearch"), aes(x=Filt, y=Hill_value, color=Filt, group=Filt, shape=Filt)) +
  scale_color_manual(values = pal3) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha=0.8, position=position_jitterdodge(jitter.width = 0.3)) +
...
q0.Vonly <- ggplot(q0.hill.df %>% filter(Method == "vsearch"), aes(x=Filt, y=Hill_value, color=Filt, group=Filt, shape=Filt)) +
  scale_color_manual(values = pal3) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha=0.8, position=position_jitterdodge(jitter.width = 0.3)) +
  #scale_y_continuous(limits = c(20,45)) +
...
plot.q0 <- plot_grid(q0.notV, q0.Vonly, nrow=1, rel_widths = c(1.8, 1))

## plot; save as 10_mockAlpha_HillNumbers_variedAxis; export at 800x800
plot_grid(plot.q0, plot.q1, plot.q2, ncol=1, rel_heights = c(1.3, 1, 1))
