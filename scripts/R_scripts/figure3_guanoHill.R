...
df <- df %>% filter(SampleType == "sample")
...
all.guano.hill <- rbind(rarefied.dada2.basic, rarefied.dada2.standard, rarefied.dada2.extra, 
...
all.guano.hill$Method <- as.factor(all.guano.hill$Method)
rm(list=ls(pattern = "rarefied*"))

write.csv(all.guano.hill, 
          file="~/Repos/tidybug/data/text_tables/guano_hillvals_table.csv", 
          quote = FALSE, 
          row.names = FALSE)
...
## Kruskal-Wallis:
kw_hill_guano_function <- function(HillVal){
  basic_dat <- all.guano.hill %>% filter(Hill_qType == HillVal, Filt=="basic")
  stand_dat <- all.guano.hill %>% filter(Hill_qType == HillVal, Filt=="standard")
  extra_dat <- all.guano.hill %>% filter(Hill_qType == HillVal, Filt=="extra")
...
q0_kw_df <- kw_hill_guano_function('q=0')
q1_kw_df <- kw_hill_guano_function('q=1')
q2_kw_df <- kw_hill_guano_function('q=2')

kw_df_stats <- rbind(q0_kw_df, q1_kw_df, q2_kw_df)
rm(q0_kw_df, q1_kw_df, q2_kw_df)

write.csv(kw_df_stats, 
          file="~/Repos/tidybug/data/text_tables/kw_guano_table.csv", 
          quote = FALSE, 
          row.names = FALSE)

## Dunn's test:
dunn_hill_guano_function <- function(HillVal){
  basic_dat <- all.guano.hill %>% filter(Hill_qType == HillVal, Filt=="basic")
  stand_dat <- all.guano.hill %>% filter(Hill_qType == HillVal, Filt=="standard")
  extra_dat <- all.guano.hill %>% filter(Hill_qType == HillVal, Filt=="extra")
...
q0_dunn_df <- dunn_hill_guano_function('q=0')
q1_dunn_df <- dunn_hill_guano_function('q=1')
q2_dunn_df <- dunn_hill_guano_function('q=2')

dunn_df_stats <- rbind(q0_dunn_df, q1_dunn_df, q2_dunn_df)
rm(q0_dunn_df, q1_dunn_df, q2_dunn_df)

write.csv(dunn_df_stats, 
          file="~/Repos/tidybug/data/text_tables/dunn_guano_table.csv", 
          quote = FALSE, 
          row.names = FALSE)


################################################################################
## Plotting diversity estimates of guano samples
################################################################################
pal3 <- c('#9f9244', '#6c42b8', '#628a47', '#a9d190')

all.guano.hill$Filt <- factor(all.guano.hill$Filt, levels = c("basic", "standard", "extra"))

all.guano.hill$log2HillVal <- log2(all.guano.hill$Hill_value)

ggplot(all.guano.hill,
       aes(x=Method, y=Hill_value, color=Method)) +
  facet_grid(Hill_qType ~ Filt) +
  geom_jitter(width = 0.2, alpha=0.1) +
  geom_boxplot(outlier.shape = NA) +
  labs(x="", y="ASV equivalents\n") +
  scale_color_manual(values=pal3) +
  scale_y_continuous(trans = "log2", 
                     breaks = c(2,8,32,128)) +
  geom_text(aes(x=Method,y=Hill_value,label=Vals),
            color='black',
            data=data.frame(
              Method=c(rep("dada2",9), rep("deblur",9), rep("vsearch",9)),
              Filt=rep(c(rep("basic",3), rep("standard",3), rep("extra",3)),3),
              Hill_qType=rep(c('q=0', 'q=1', 'q=2'),9),
              Hill_value=rep(300,27),
              Vals=c("a", "a", "a", 
                     "a", "a", "a", 
                     "a", "a", "a",
                     "a", "a", "b",
